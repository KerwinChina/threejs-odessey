<template>
  <div class="family">
    ...
    <div class="mascot-and-slider" :class="{'is-relative': ['living-room', 'living-room-out', 'kitchen', 'bed-room'].includes(data.currentRoom), 'more-slider': data.sliders && data.sliders.length > 5}">
      ...
      <Mascot type="float" v-if="data.showMascot && data.currentKeyframe === 'float'" :position="{'bottom': '16px'}" />
      <Mascot type="intro" v-if="data.currentRoom === 'hall-0'" :position="{'bottom': '-180px'}" />
...
import { rooms, markers, roomLabels } from '@/views/family/data';
